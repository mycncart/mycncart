...
class ModelCmsBlog extends Model {
	public function addBlog($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "blog SET featured = '" . (int)$data['featured'] . "', hits = '" . (int)$data['hits'] . "', created = '" . $this->db->escape($data['created']) . "', video_code = '" . $this->db->escape($data['video_code']) . "', user_id = '" . (int)$data['user_id'] . "', blog_category_id = '" . (int)$data['blog_category_id'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW()");
		...
		foreach ($data['blog_description'] as $language_id => $value) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "blog_description SET blog_id = '" . (int)$blog_id . "', language_id = '" . (int)$language_id . "', title = '" . $this->db->escape($value['title']) . "', brief = '" . $this->db->escape($value['brief']) . "',  description = '" . $this->db->escape($value['description']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', tag = '" . $this->db->escape($value['tag']) . "'");
		}
	...
	public function editBlog($blog_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "blog SET featured = '" . (int)$data['featured'] . "', hits = '" . (int)$data['hits'] . "', created = '" . $this->db->escape($data['created']) . "', user_id = '" . (int)$data['user_id'] . "', video_code = '" . $this->db->escape($data['video_code']) . "', blog_category_id = '" . (int)$data['blog_category_id'] . "', status = '" . (int)$data['status'] . "',  sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE blog_id = '" . (int)$blog_id . "'");
