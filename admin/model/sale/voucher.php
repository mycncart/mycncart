	...
	public function sendVoucher($voucher_id) {
		...
		if ($voucher_info) {
			...
			if ($order_info) {
				...
				if (filter_var($voucher_info['to_email'], FILTER_VALIDATE_EMAIL)) {
					$mail = new Mail();
			...
			// If voucher does not belong to an order
				...
				$data['text_greeting'] = sprintf($this->language->get('text_greeting'), $this->currency->format($voucher_info['amount'], $this->session->data['currency']));
