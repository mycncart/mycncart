...
class ControllerDesignSeoUrl extends Controller {
	...
	protected function getList() {
		...
		$data['product_url'] = $this->url->link('design/seo_url/product_url', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['information_url'] = $this->url->link('design/seo_url/information_url', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['category_url'] = $this->url->link('design/seo_url/category_url', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['manufacturer_url'] = $this->url->link('design/seo_url/manufacturer_url', 'user_token=' . $this->session->data['user_token'] . $url, true);
	...
	public function product_url() {
		...
		$results = $this->model_design_seo_url->getProducts();
		
		foreach ($results as $result) {
			$string = $this->replace_specialChar($result['name']);
			$string = str_replace(' ','-',trim($string));
			$keyword = str_replace('--','-',trim($string));
			$keyword = strtolower($keyword);
			$query = 'product_id='.$result['product_id'];
			$exist_keyword = $this->model_design_seo_url->getUrlAliasByKeyword($keyword, $result['language_id'], $result['store_id']);
			$exist_query = $this->model_design_seo_url->getUrlAliasByQuery($query, $result['language_id'], $result['store_id']);
			
			//如果不存在query，但存在相同的keyword，则为keyword添加id
			if ((!$exist_query) && $exist_keyword) {
			...
			} elseif ((!$exist_keyword) && (!$exist_query)) {
	...
	public function category_url() {
		...
		$results = $this->model_design_seo_url->getCategories();
		
		foreach ($results as $result) {
			$string = $this->replace_specialChar(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8'));
			$string = str_replace(' ','-',trim($string));
			$keyword = str_replace('--','-',trim($string));
			$keyword = strtolower($keyword);
			$query = 'category_id='.$result['category_id'];
			$exist_keyword = $this->model_design_seo_url->getUrlAliasByKeyword($keyword, $result['language_id'], $result['store_id']);
			$exist_query = $this->model_design_seo_url->getUrlAliasByQuery($query, $result['language_id'], $result['store_id']);
	...
	public function information_url() {
		$this->load->language('design/seo_url');
		...
		$results = $this->model_design_seo_url->getInformations();
		
		foreach ($results as $result) {
			$string = $this->replace_specialChar($result['title']);
			$string = str_replace(' ','-',trim($string));
			$keyword = str_replace('--','-',trim($string));
			$keyword = strtolower($keyword);
	...
	public function manufacturer_url() {
		$this->load->language('design/seo_url');
		...
		$results = $this->model_design_seo_url->getManufacturers();
		...
		foreach ($results as $result) {
			...
			foreach ($languages as $language) {
				...
				$exist_keyword = $this->model_design_seo_url->getUrlAliasByKeyword($keyword, $language['language_id'], $result['store_id']);
				$exist_query = $this->model_design_seo_url->getUrlAliasByQuery($query, $language['language_id'], $result['store_id']);
				
				//如果不存在query，但存在相同的keyword，则为keyword添加id
				if ((!$exist_query) && $exist_keyword) {
					
					$data = array(
						'keyword'	=> $keyword . '-' . $result['manufacturer_id'],
						'query'	=> $query,
