...
"version":"1.0","format":"json"}';
	...
	protected function getForm() {
		...
		$data['entry_model'] = $this->language->get('entry_model');
		$data['entry_shipping_origin'] = $this->language->get('entry_shipping_origin');
		...
		$data['entry_maxbuy'] = $this->language->get('entry_maxbuy');
		
		$data['entry_refer_price'] = $this->language->get('entry_refer_price');
		$data['entry_refer_weight'] = $this->language->get('entry_refer_weight');
		$data['entry_p2u_url'] = $this->language->get('entry_p2u_url');
		...
		$data['help_feature'] = $this->language->get('help_feature');
		$data['help_maxbuy'] = $this->language->get('help_maxbuy');
		...
		$data['shipping_origins'] = $this->shipping_origins;
		foreach ($data['shipping_origins'] as $so_key=>$so_val) {
			$data['shipping_origins'][$so_key] = $this->language->get($so_val);
		}
		...
		if (isset($this->error['shipping_origin'])) {
			$data['error_shipping_origin'] = $this->error['shipping_origin'];
		} else {
			$data['error_shipping_origin'] = '';
		}
		...
		if (!isset($this->request->get['product_id'])) {
			$data['action'] = $this->url->link('weidian/product/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('weidian/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('weidian/product', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['product_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$product_info = $this->model_weidian_product->getProduct($this->request->get['product_id']);
		}
		...
		if (isset($this->request->post['product_description'])) {
			...
			$data['product_description'] = $this->model_weidian_product->getProductDescriptions($this->request->get['product_id']);
		} else {
		...
		if (!empty($product_info)) {
			...
			if($product_info['shipping_origin_id'] == 2) {
				$data['p2u_url'] = 'http://www.post2u.co.nz/'.$product_info['fromurl'];
			}else{
				$data['p2u_url'] = $product_info['fromurl'];
			}
		} else {
			$data['p2u_url'] = '';
		}
		...
		if (isset($this->request->post['maxbuy'])) {
			$data['maxbuy'] = $this->request->post['maxbuy'];
		} elseif (!empty($product_info)) {
			$data['maxbuy'] = $product_info['maxbuy'];
		} else {
			$data['maxbuy'] = '';
		}

		/*
		if (isset($this->request->post['shipping_origin'])) {
			$data['shipping_origin'] = $this->request->post['shipping_origin'];
		} elseif (!empty($product_info)) {
			$data['shipping_origin'] = $product_info['shipping_origin'];
		} else {
			//$data['shipping_origin'] = key($this->shipping_origins); // Notice: Indirect modification of overloaded property ControllerWeidianProduct::$shipping_origins has no effect in D:\develop\projects\relibot_prjs\xinao\xinao\admin\controller\weidian\product.php on line 791
			$data['shipping_origin'] = key($data['shipping_origins']); // OK
		}
		*/
		
		$this->load->model('localisation/shipping_origin');

		$data['shipping_origins'] = $this->model_localisation_shipping_origin->getShippingOrigins();
		
		if (isset($this->request->post['shipping_origin_id'])) {
			$data['shipping_origin_id'] = $this->request->post['shipping_origin_id'];
		} elseif (!empty($product_info)) {
			$data['shipping_origin_id'] = $product_info['shipping_origin_id'];
		}
		...
		if (isset($this->request->post['refer_price'])) {
			$data['refer_price'] = $this->request->post['refer_price'];
		} elseif (!empty($product_info)) {
			$data['refer_price'] = $product_info['refer_price'];
		} else {
			$data['refer_price'] = '';
		}
		...
		if (!empty($product_info)) {
			$data['refer_weight'] = $product_info['refer_weight'];
		} else {
			$data['refer_weight'] = '';
		}
		...
		if (isset($this->request->post['product_store'])) {
			...
			$data['product_store'] = $this->model_weidian_product->getProductStores($this->request->get['product_id']);
		} else {
		...
		$this->load->model('weidian/recurring');

		$data['recurrings'] = $this->model_weidian_recurring->getRecurrings();

		if (isset($this->request->post['product_recurrings'])) {
			$data['product_recurrings'] = $this->request->post['product_recurrings'];
		} elseif (!empty($product_info)) {
			$data['product_recurrings'] = $this->model_weidian_product->getRecurrings($product_info['product_id']);
		} else {
		...
		$this->load->model('weidian/manufacturer');
		...
		if (isset($this->request->post['manufacturer'])) {
			...
			$manufacturer_info = $this->model_weidian_manufacturer->getManufacturer($product_info['manufacturer_id']);
		...
		if (isset($this->request->post['product_category'])) {
			...
			$categories = $this->model_weidian_product->getProductCategories($this->request->get['product_id']);
		} else {
			$categories = array();
		}
		...
		// features活动商品主题
		$this->load->model('weidian/feature');

		if (isset($this->request->post['product_feature'])) {
			$features = $this->request->post['product_feature'];
		} elseif (isset($this->request->get['product_id'])) {
			$features = $this->model_weidian_product->getProductFeatures($this->request->get['product_id']);
		} else {
			$features = array();
		}
		...
		foreach ($features as $feature_id) {
			$feature_info = $this->model_weidian_feature->getFeature($feature_id);
		...
		// Filters
		$this->load->model('weidian/filter');

		if (isset($this->request->post['product_filter'])) {
			$filters = $this->request->post['product_filter'];
		} elseif (isset($this->request->get['product_id'])) {
			$filters = $this->model_weidian_product->getProductFilters($this->request->get['product_id']);
		} else {
			$filters = array();
		}
		...
		foreach ($filters as $filter_id) {
			$filter_info = $this->model_weidian_filter->getFilter($filter_id);
		...
		// Attributes
		$this->load->model('weidian/attribute');

		if (isset($this->request->post['product_attribute'])) {
			$product_attributes = $this->request->post['product_attribute'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_attributes = $this->model_weidian_product->getProductAttributes($this->request->get['product_id']);
		} else {
		...
		foreach ($product_attributes as $product_attribute) {
			$attribute_info = $this->model_weidian_attribute->getAttribute($product_attribute['attribute_id']);
		...
		if (isset($this->request->post['product_option'])) {
			...
			$product_options = $this->model_weidian_product->getProductOptions($this->request->get['product_id']);
		} else {
		...
		foreach ($data['product_options'] as $product_option) {
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
				if (!isset($data['option_values'][$product_option['option_id']])) {
					$data['option_values'][$product_option['option_id']] = $this->model_weidian_option->getOptionValues($product_option['option_id']);
				}
			}
		}
		...
		if (isset($this->request->post['product_discount'])) {
			...
			$product_discounts = $this->model_weidian_product->getProductDiscounts($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_special'])) {
			...
			$product_specials = $this->model_weidian_product->getProductSpecials($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_image'])) {
			...
			$product_images = $this->model_weidian_product->getProductImages($this->request->get['product_id']);
		} else {
		...
		// Downloads
		$this->load->model('weidian/download');

		if (isset($this->request->post['product_download'])) {
			$product_downloads = $this->request->post['product_download'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_downloads = $this->model_weidian_product->getProductDownloads($this->request->get['product_id']);
		} else {
		...
		foreach ($product_downloads as $download_id) {
			$download_info = $this->model_weidian_download->getDownload($download_id);
		...
		if (isset($this->request->post['product_related'])) {
			...
			$products = $this->model_weidian_product->getProductRelated($this->request->get['product_id']);
		} else {
			$products = array();
		}
		...
		foreach ($products as $product_id) {
			$related_info = $this->model_weidian_product->getProduct($product_id);
		...
		if (isset($this->request->post['product_reward'])) {
			...
			$data['product_reward'] = $this->model_weidian_product->getProductRewards($this->request->get['product_id']);
		} else {
		...
		if (isset($this->request->post['product_layout'])) {
			...
			$data['product_layout'] = $this->model_weidian_product->getProductLayouts($this->request->get['product_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('weidian/product_form.tpl', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'weidian/product')) {
		...
		/*
		if ((utf8_strlen($this->request->post['shipping_origin']) < 1) || (utf8_strlen($this->request->post['shipping_origin']) > 64)) {
			$this->error['shipping_origin'] = $this->language->get('error_shipping_origin');
		}
		*/

		if (utf8_strlen($this->request->post['keyword']) > 0) {
			$this->load->model('weidian/url_alias');

			$url_alias_info = $this->model_weidian_url_alias->getUrlAlias($this->request->post['keyword']);
          ...
          if (isset($this->request->get['image'])) {
            ...
            return;
      		...
      		if (!is_file(DIR_IMAGE . $this->request->get['image'])) {
      			if (is_file(DIR_IMAGE ."weidian/". $this->request->get['image'])) {
        			$this->request->get['image'] = 'weidian/'.$this->request->get['image'];
        		} else {
          		return;
        		}
      		}                
