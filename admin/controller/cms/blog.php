...
class ControllerCmsBlog extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('cms/blog', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('cms/blog', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('cms/blog/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['copy'] = $this->url->link('cms/blog/copy', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('cms/blog/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			...
			$data['blogs'][] = array(
				...
				'edit'       => $this->url->link('cms/blog/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $result['blog_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_title'] = $this->url->link('cms/blog', 'token=' . $this->session->data['token'] . '&sort=pd.title' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('cms/blog', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, 'SSL');
		$data['sort_order'] = $this->url->link('cms/blog', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('cms/blog', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('cms/blog_list.tpl', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['blog_id'])) {
			$data['action'] = $this->url->link('cms/blog/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('cms/blog/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $this->request->get['blog_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('cms/blog', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$this->response->setOutput($this->load->view('cms/blog_form.tpl', $data));
	}
