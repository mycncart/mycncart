...
class ModelInstallInstall extends Model {
	public function database($data) {
		...
		if ($lines) {
			...
			$db->query("INSERT INTO `" . $data['db_prefix'] . "user` SET user_id = '1', user_group_id = '1', username = '" . $db->escape($data['username']) . "', salt = '" . $db->escape($salt = token(9)) . "', password = '" . $db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', firstname = 'Admin Name', lastname = '', email = '" . $db->escape($data['email']) . "', status = '1', date_added = NOW()");
