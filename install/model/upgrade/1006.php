...
class ModelUpgrade1006 extends Model {
	public function upgrade() {
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "setting` SET `value` = 'zh-CN' WHERE `key` = 'config_language' AND `value` = 'cn'");
		$this->db->query("UPDATE `" . DB_PREFIX . "setting` SET `value` = 'zh-CN' WHERE `key` = 'config_admin_language' AND `value` = 'cn'");
		$this->db->query("UPDATE `" . DB_PREFIX . "language` SET `code` = 'en-gb' WHERE `code` = 'en'");
		$this->db->query("UPDATE `" . DB_PREFIX . "language` SET `code` = 'zh-CN' WHERE `code` = 'cn'");
		$this->db->query("UPDATE `" . DB_PREFIX . "language` SET `code` = 'zh-HK' WHERE `code` = 'zh_HK'");
		...
		if (is_file(DIR_MYCNCART . 'config.php')) {
			$files = glob(DIR_MYCNCART . '{config.php,*/config.php}', GLOB_BRACE);
