	...
	public function mysql() {
		...
		foreach ($query->rows as $result) {
			if (preg_match('/^(a:)/', $result['data'])) {
				...
				$this->db->query("UPDATE `" . DB_PREFIX . "affiliate_activity` SET `data` = '" . $this->db->escape(json_encode($data)) . "' WHERE `activity_id` = '" . (int)$result['affiliate_activity_id'] . "'");
			}
		}
		...
		foreach ($query->rows as $result) {
			if (preg_match('/^(a:)/', $result['data'])) {
				$data = unserialize($result['data']);
				$this->db->query("UPDATE `" . DB_PREFIX . "customer_activity` SET `data` = '" . $this->db->escape(json_encode($data)) . "' WHERE `activity_id` = '" . (int)$result['customer_activity_id'] . "'");
			}
		}	
