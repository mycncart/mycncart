...
class ControllerExtensionPaymentPPExpress extends Controller {
	...
	public function expressReturn() {
		...
		if ($this->session->data['paypal']['guest'] == true) {
			...
			$this->session->data['guest']['fullname'] = trim($result['FIRSTNAME']);
			...
			$this->session->data['guest']['payment']['fullname'] = trim($result['FIRSTNAME']);
			...
			if ($this->cart->hasShipping()) {
				...
				$this->session->data['guest']['payment']['address'] = $result['PAYMENTREQUEST_0_SHIPTOSTREET'];
				...
				$this->session->data['guest']['shipping']['address'] = $result['PAYMENTREQUEST_0_SHIPTOSTREET'];
			...
			if ($this->cart->hasShipping()) {
				...
				if ($match == false) {
					...
					$address_data = array(
						...
						'company_id' => '',
						'tax_id'     => '',
						'address'  => $result['PAYMENTREQUEST_0_SHIPTOSTREET'],
	...
	public function expressComplete() {
		...
		if ($redirect == '') {
			...
			$data['payment_address'] = isset($payment_address['address']) ? $payment_address['address'] : '';
	...
	public function checkout() {
		...
		if ($this->cart->hasShipping()) {
			...
			$data_shipping = array(
				'PAYMENTREQUEST_0_SHIPTONAME'        => html_entity_decode($order_info['shipping_fullname'], ENT_QUOTES, 'UTF-8'),
				'PAYMENTREQUEST_0_SHIPTOSTREET'      => html_entity_decode($order_info['shipping_address'], ENT_QUOTES, 'UTF-8'),
