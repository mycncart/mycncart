...
class ModelExtensionPaymentPilibaba extends Model {
	...
	public function updateOrderInfo($data, $order_id) {
		...
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET `fullname` = '" . $this->db->escape($data['fullname']) . "', `email` = '" . $this->db->escape($data['email']) . "', `telephone` = '" . $this->db->escape($data['mobile']) . "', `payment_fullname` = '" . $this->db->escape($data['fullname']) . "', `payment_address` = '" . $this->db->escape($data['address']) . "', `payment_city` = '" . $this->db->escape($data['city']) . "', `payment_postcode` = '" . $this->db->escape($data['zipcode']) . "', `payment_country` = '" . $this->db->escape($data['country']) . "', `payment_zone` = '" . $this->db->escape($data['district']) . "', `shipping_fullname` = '" . $this->db->escape($data['fullname']) . "', `shipping_address` = '" . $this->db->escape($data['address']) . "', `shipping_city` = '" . $this->db->escape($data['city']) . "', `shipping_postcode` = '" . $this->db->escape($data['zipcode']) . "', `shipping_country` = '" . $this->db->escape($data['country']) . "', `shipping_zone` = '" . $this->db->escape($data['district']) . "', `date_modified` = NOW() WHERE `order_id` = '" . (int)$order_id . "'");
	}
