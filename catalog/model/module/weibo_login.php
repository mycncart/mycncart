...
class ModelModuleWeiBoLogin extends Model {
	public function getTokens($code) {
		if ($this->config->get('weibo_login_sandbox')) {
		...
		$request .= '&code=' . $code;
		$request .= '&redirect_uri=' . urlencode($this->url->link('module/weibo_login/login', '', true));

		$additional_opts = array(
			CURLOPT_USERPWD    => $this->config->get('weibo_login_client_id') . ':' . $this->config->get('weibo_login_secret'),
			CURLOPT_POST       => true,
